...
function [Prod,ProdBase] = MakeProd(P,C)
  ...
  if strcmp(P.ProdFn,'data')
    ...
    ProdBase  = Prod;
  elseif strcmp(P.Scheme,'customProd')
    [X,Y] = meshgrid(1:50,1:50);
    temp      = arrayfun(P.ProdFn,Y,X);
    ...
    for ix = 1:C.LenGrid
      ...
      for iy = 1:C.LenGrid
        ...
        ProdBase(ix,iy) = interp2(linspace(0,1,50),linspace(0,1,50),temp,X,Y);
        for iz = 1:C.LenGridZ
          %This is Production including the shock
          Prod(ix,iy,iz) = ProdBase(ix,iy) + C.GridZeta(iz);
        end
      end
    end
  else
